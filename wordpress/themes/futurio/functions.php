...
if ( !function_exists( 'futurio_excerpt_length' ) ) :
	...
	function futurio_excerpt_length( $length ) {
		if ( is_home() ) { 
			return get_theme_mod( 'blog_archive_excerpt', '15' ); 
		} else { 
			return $length; 
		} 
	}

	add_filter( '_length', 'futurio_excerpt_length', 999 );
		...
		<?php if ( $post->have_posts() ) : ?>
			<div class="row" >
				<?php while ( $post->have_posts() ) : ?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'col-sm-' . absint( $columns ) ); ?>>
